...
### Go Backend (Optional)
- **Multi-Server Support**: Manage connections to multiple Veeam products from a single UI
- **Response Caching**: Intelligent caching layer reduces API calls with configurable TTL
- **Rate Limiting**: Built-in rate limiting prevents API throttling
- **Centralized Token Management**: Automatic authentication and token refresh
- **Encrypted Credential Storage**: AES-256-GCM encryption for secure credential storage
- **Administration UI**: Web-based interface for managing servers and cache
...
The easiest way to get started - zero manual configuration required:
...
# Copy the dist folder to your target machine and run:
.\Start-VeeamUI.ps1
```
...
#### macOS
```bash
# Build the standalone package (auto-detects Intel vs Apple Silicon)
./build/build-macos.sh

# Or build a universal binary for both architectures
...
On first run:
1. **Encryption keys are generated automatically** - no need to create a passphrase
2. **The Setup Wizard opens in your browser** - configure VBR, VRO, VBM servers through a friendly UI
3. **Credentials are encrypted and stored** - in a secure SQLite database

See [build/README.md](./build/README.md) for detailed build options and cross-platform instructions.
...
2. Create a `.env.local` file (see Environment Variables section below)
...
When using the Go backend, you only need minimal environment configuration:

```env
# Go Backend URL (required)
NEXT_PUBLIC_GO_BACKEND_URL=http://localhost:8080

# Encryption (required - auto-generated by Start-VeeamUI.ps1)
MASTER_PASSPHRASE=your-secure-passphrase-min-16-chars

# Allow self-signed certificates
NODE_TLS_REJECT_UNAUTHORIZED=0
```

Server connections (VBR, VRO, VBM) are configured through:
- **Setup Wizard** on first run
- **Administration > Servers > Connections** for ongoing management

The Go backend provides:
- **Response Caching**: Reduces API calls to Veeam products with configurable TTL
- **Rate Limiting**: Prevents API throttling (especially for VBM's 1 req/sec limit)
- **Multi-Server Support**: Connect multiple VBR/VRO/VBM servers from one UI
- **Token Management**: Centralized authentication with automatic refresh
- **Encrypted Storage**: Credentials stored securely with AES-256-GCM encryption

See [backend/README.md](./backend/README.md) for Go backend details.
