#!/bin/bash
#
# Build script for Veeam Single-UI macOS standalone package
#
# SYNOPSIS
...
#     macOS package including:
#     - Next.js frontend (static export)
#     - Go backend compiled for macOS (Darwin)
#
# OPTIONS
...
#     --arch              Target architecture: amd64 (Intel), arm64 (Apple Silicon) (default: auto-detect)
#     --universal         Build universal binary (both amd64 and arm64)
#     -h, --help          Show this help message
#
# EXAMPLES
#     ./build-macos.sh
#     ./build-macos.sh --config debug --output ~/builds/veeam-ui
...
#     - Go 1.22 or later
#     - Xcode Command Line Tools (for universal builds)
#
...
if [[ -z "$TARGET_ARCH" ]] && [[ "$UNIVERSAL" == false ]]; then
...
if [[ "$UNIVERSAL" == false ]] && [[ "$TARGET_ARCH" != "amd64" && "$TARGET_ARCH" != "arm64" ]]; then
...
print_header "Veeam Single-UI macOS Build"
...
if [[ "$SKIP_BACKEND" == false ]]; then
    ...
    # Check lipo for universal builds
    if [[ "$UNIVERSAL" == true ]]; then
        print_step "Checking lipo (for universal binary)..."
        if command -v lipo &> /dev/null; then
            print_success "lipo found"
        else
            print_error "lipo not found. Install Xcode Command Line Tools: xcode-select --install"
            exit 1
        fi
    fi
fi
...
if [[ "$SKIP_BACKEND" == false ]]; then
    ...
    if [[ "$UNIVERSAL" == true ]]; then
        ...
        print_step "Compiling Go backend for darwin/amd64..."
        export GOARCH=amd64
        if [[ -n "$LDFLAGS" ]]; then
            go build -ldflags="$LDFLAGS" -o "${EXE_PATH}-amd64" ./cmd/server
        else
            go build -o "${EXE_PATH}-amd64" ./cmd/server
        fi
        
        print_step "Compiling Go backend for darwin/arm64..."
        export GOARCH=arm64
        if [[ -n "$LDFLAGS" ]]; then
            go build -ldflags="$LDFLAGS" -o "${EXE_PATH}-arm64" ./cmd/server
        else
            go build -o "${EXE_PATH}-arm64" ./cmd/server
        fi
        
        print_step "Creating universal binary..."
        lipo -create -output "$EXE_PATH" "${EXE_PATH}-amd64" "${EXE_PATH}-arm64"
        rm "${EXE_PATH}-amd64" "${EXE_PATH}-arm64"
        
        print_success "Universal binary created"
    else
        # Build for single architecture
        print_step "Compiling Go backend for darwin/$TARGET_ARCH..."
...
#!/bin/bash
#
# Start Veeam Single-UI application on macOS
#
...
# Create launchd plist for macOS service
cat > "$CONFIG_OUTPUT/com.veeam.single-ui.plist" << EOF
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.veeam.single-ui</string>
    <key>ProgramArguments</key>
    <array>
        <string>/usr/local/veeam-ui/start-veeam-ui.sh</string>
    </array>
    <key>WorkingDirectory</key>
    <string>/usr/local/veeam-ui</string>
    <key>KeepAlive</key>
    <true/>
    <key>RunAtLoad</key>
    <true/>
    <key>StandardOutPath</key>
    <string>/usr/local/veeam-ui/logs/stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/usr/local/veeam-ui/logs/stderr.log</string>
...
print_success "Created com.veeam.single-ui.plist (launchd)"

# Create README
cat > "$OUTPUT_PATH/README.md" << 'EOF'
# Veeam Single-UI - macOS Build
...
- `config/` - Sample configuration and launchd plist
...
## Installation as a Service (launchd)

```bash
# Copy files to /usr/local
sudo mkdir -p /usr/local/veeam-ui
sudo cp -r * /usr/local/veeam-ui/

# Create logs directory
sudo mkdir -p /usr/local/veeam-ui/logs

# Set permissions
sudo chown -R $(whoami) /usr/local/veeam-ui
chmod 600 /usr/local/veeam-ui/.env

# Install and start service
sudo cp config/com.veeam.single-ui.plist /Library/LaunchDaemons/
...
# View logs
tail -f /usr/local/veeam-ui/logs/stdout.log
```

To stop the service:
```bash
sudo launchctl unload /Library/LaunchDaemons/com.veeam.single-ui.plist
```
...
3. **Keychain**: For enhanced security, consider storing the passphrase in macOS Keychain
...
- Allow terminal in System Preferences > Security & Privacy > Privacy > Developer Tools
...
### Firewall issues
- Allow connections in System Preferences > Security & Privacy > Firewall

### Apple Silicon (M1/M2) issues
- The binary should work on both Intel and Apple Silicon
- If built with --universal, it's a fat binary supporting both architectures
EOF
...
echo ""
echo "Next Steps:"
echo "  1. Run ./start-veeam-ui.sh to start the application"
echo "  2. Follow the Setup Wizard to configure your Veeam servers"
echo ""
